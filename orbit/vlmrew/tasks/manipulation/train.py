...
def main():
    ...
    env = RLTaskEnv(cfg=env_cfg, render_mode="rgb_array" if args_cli.video else None)

    # agent_cfg = load_cfg_from_registry(args_cli.task, "sb3_cfg_entry_point")
    # Path to the yaml file that has the agent parameters
    ...
    # fix the random seed based on the command line argument
    if args_cli.seed is not None:
        agent_cfg["seed"] = args_cli.seed

    # directory for logging into
    log_dir = os.path.join(
        "/home/levi/projects/orbit.vlmrew/logs", "sb3", args_cli.task, datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    )
    ...
    if "normalize_input" in agent_cfg:
        print("[INFO] Normalize input observations.")
        env = VecNormalize(
            env,
            training=True,
